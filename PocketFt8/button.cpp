...
void LPF_SendRegister(uint8_t reg, uint8_t val){ 
  ...
  if(latch){
    LPF_write((1U << io)| 0x0000); delay(LATCH_TIME); LPF_write(0x0000); // set latch wired to io port
  } else {
    if(io == 0xff){ LPF_init(); for(int io = 0; io != 16; io++) LPF_set_latch(io, latch); } // reset all latches
    else { LPF_write( (~(1U << io))| 0x0002); delay(LATCH_TIME); LPF_write(0x0000); } // reset latch wired to io port
  }
  }
...
void LPF_set_lpf(uint8_t f){
  ...
  if(prev_lpf_io != lpf_io){ LPF_set_latch(prev_lpf_io, false); LPF_set_latch(lpf_io, true); prev_lpf_io = lpf_io; };  // set relay
}
